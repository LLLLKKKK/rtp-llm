...
def build_remote_setup_command(rootdir: Path) -> str:
    ...
    return (
        ...
        # prepare_venv runs, so the current session usually has space.
        # If the worker is still below 50 GiB free, evict 6h-old venv/cache
        # dirs; if it is still below 20 GiB, evict 1h-old dirs. The age gates
        # avoid deleting other currently running actions, while recovering
        # workers that accumulated many same-day CUDA/flashinfer venvs.
...
def build_runtime_config(
    ...
    *,
    ...
    reapi_gpu_type = _to_reapi_gpu_type(gpu_request.gpu_type)
    if not reapi_gpu_type.startswith(("MI308X", "PPU-")):
